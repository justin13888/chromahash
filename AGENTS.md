...
This is a **monorepo** with four language implementations of the same format specification.
...
- All four implementations MUST produce identical output for the same input — the spec in `spec/` is the source of truth
...
- Use conventional commits: `type(scope): description` — scope = `rust`, `ts`, `kotlin`, `swift`, or `spec`
