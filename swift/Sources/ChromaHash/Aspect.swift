...
func deriveGrid(_ aspectByte: Int, _ baseN: Int) -> (Int, Int) {
  ...
  if ratio >= 1.0 {
    ...
    nx = min(Int(roundHalfAwayFromZero(base * sqrt(scale))), nxCap)
    ny = Int(roundHalfAwayFromZero(base * base / Double(nx)))
  } else {
    let scale = min(1.0 / ratio, 16.0)
    ny = min(Int(roundHalfAwayFromZero(base * sqrt(scale))), nxCap)
    nx = Int(roundHalfAwayFromZero(base * base / Double(ny)))
  }
